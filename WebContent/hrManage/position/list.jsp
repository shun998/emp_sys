...
<DIV>
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=request.getContextPath()%>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">职位管理 </SPAN><SPAN 
    ...
    vAlign=top align=middle>
          <DIV>
             <DIV ALIGN="right">
              <input type="button" value="职位添加" onclick="window.location.href='<%=request.getContextPath()%>/hrManage/position.do?method=add'"/>
...
<!--                   <TH class=gridViewHeader scope=col>薪金ID</TH> -->
                  ...
                  <TH class=gridViewHeader scope=col>职位名称</TH>
...
<%--                   <TD class=gridViewItem><%=map.get("pay_id") %></TD> --%>
                  ...
                  <TD class=gridViewItem><%=map.get("position_name") %></TD>
                  <TD class=gridViewItem><%=map.get("position_remark") %></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="<%=request.getContextPath() %>/hrManage/position.do?method=edit&id=<%=map.get("position_id")%>">修改</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            id=ctl00_ContentPlaceHolder2_GridView1_ctl02_LinkButton1 
            onclick="return confirm('确定要删除吗？');" 
            href="<%=request.getContextPath() %>/hrManage/position.do?method=delete&id=<%=map.get("position_id")%>">删除</A> </TD>
                </TR>
                <%} %>
