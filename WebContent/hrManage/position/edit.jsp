			...
			<TBODY>
				<TR
					...
					<TD><SPAN
						...
						style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=request.getContextPath()%>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">职位修改
					</SPAN><SPAN
				...
				<TR>
					...
					<TD
						...
						<DIV>
							<%
      	Map position = (Map)request.getAttribute("map");
          %>
							<form name="addForm" method="post"
								action="<%=request.getContextPath()%>/hrManage/position.do?method=update&id=<%=position.get("position_id") %>" onSubmit="return checkFrom()">
...
%>
							<option value="<%=map.get("depart_id") %>" <%=map.get("depart_id").equals(position.get("depart_id")) ? "selected" : "" %>><%=map.get("depart_name") %></option>
							<%
		}
							%>
						</select>
										...
										<TR>
										  ...
										  <TD class=gridViewItem><input type="text" name="position_name" style="width:200px;" onBlur="checkFr()" value="<%=position.get("position_name") %>"/><font id="departNameFont" size="-1" color="#FF0000" style="visibility: hidden;">*职位名称为空&nbsp;</font>
											</TD>
										</TR>
										<TR>
											<TD class=gridViewItem style="WIDTH: 50px"><IMG
										  ...
										  <TD class=gridViewItem><input type="text" name="position_remark" style="width:200px;" onBlur="checkFr()" value="<%=position.get("position_remark") %>"/>
											  <font id="departNameFont" size="-1" color="#FF0000" style="visibility: hidden;">*职位名称为空</font>											  </TD>
										</TR>
										...
										<TR>
											...
											<TD class=gridViewItem><input type="button" value="返回"
												onclick="window.location.href='<%=request.getContextPath()%>/hrManage/position.do?method=query'" />
											</TD>
										</TR>
