...
function checkFr(){
	...
	if(isPassEmpty = 1){
		document.getElementById("usePassFont").style.visibility = "visible";
		return false;
	}
...
function checkFrom(){ 
	...
	if(isNameEmpty == 0 & isPassEmpty == 0) {
		return true;
	} else {
		return false;
	}
...
<!--                 %>  -->
  ...
  height=47>
    ...
    vAlign=top align=middle>
          <DIV>
          <form name="addForm" method="post" action="<%=request.getContextPath() %>/hrManage/manager.do?method=edit&manager_id=<%=map.get("manager_id") %>" onsubmit="return checkFrom()">
      ...
      border=1>
            ...
            src="<%=request.getContextPath()%>/images/employee/bg_users.gif"></TD>
                  ...
                  <TD class=gridViewItem><input type="text" name="manager_name" id="manager_name" onblur="checkFr()" value="<%=map.get("manager_name") %>"/><font id="userNameFont" size="-1" color="#FF0000" style="visibility: hidden;">*用户名为空</font></TD>
                 </TR>
                  <TR>
                  <TD class=gridViewItem style="WIDTH: 50px"><IMG 
            src="<%=request.getContextPath()%>/images/employee/bg_users.gif"></TD>
                  <TD class=gridViewItem>管理员密码:</TD>
                  <TD class=gridViewItem><input type="text" name="manager_pass" id="manager_pass" onblur="checkFr()" value="<%=map.get("manager_pass") %>"/><font id="userPassFont" size="-1" color="#FF0000" style="visibility: hidden;">*密码为空 &nbsp;</font></TD>
                 </TR>
                  <TR>
                  <TD class=gridViewItem style="WIDTH: 50px"><IMG 
            src="<%=request.getContextPath()%>/images/employee/bg_users.gif"></TD>
