			...
			<TBODY>
				<TR
					...
					<TD><SPAN
						...
						style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=request.getContextPath()%>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">薪金修改
					</SPAN><SPAN
					...
					<TD
						...
						<DIV>
							...
							<form name="addForm" method="post"
								action="<%=request.getContextPath()%>/hrManage/pay.do?method=edit&id=<%=map.get("pay_id") %>"
								onsubmit="return checkFrom(addForm)">
								<input type="hidden" name="pay_id" value="<%=map.get("pay_id") %>" />
								<TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1
									...
									<TBODY>
										...
										<TR>
											...
											<input type="hidden"
												 name="employee_id" size="20" value="<%=map.get("employee_id") %>">
											<TD class=gridViewItem>
											<input type="text"
												 name="employee_name" size="20" value="<%=map.get("employee_name") %>" readonly>
<!-- 											<select -->
<!-- 										name="employee_id" id="select" style="width:160 "> -->
<!-- 											<option value="%">---请选择---</option> -->
											
<%-- 											<% --%>
<!-- 											List listemployee=(List)request.getAttribute("listemployee");  -->
<!-- 											for(int i=0;i<listemployee.size();i++){  -->
<!--  												Map employe_map =(Map)listemployee.get(i);  -->
<!--  												String employee_id=(String)employe_map.get("employee_id");  -->
<!-- 												String employee_name=(String)employe_map.get("employee_name");  -->
											
<!-- 											%>  -->
<%-- 											<option value="<%=employee_id%>"<%=employee_id.equals(map.get("employee_id"))?"selected":""%>  ><%=employee_name %></option> --%>
<%-- 											<%} %> --%>
<!-- 											</select> -->
										...
										<TR>
											...
											<TD class=gridViewItem><input type="text"
												id="basic" name="pay_basic" size="20" value="<%=map.get("pay_basic") %>" onchange="calcu()" onblur="show()">
										...
										<TR>
											...
											<TD class=gridViewItem><input type="text"
												id="perf" name="pay_perf" size="20" value="<%=map.get("pay_perf")%>" onchange="calcu()" onblur="show()">
										...
										<TR>
											...
											<TD class=gridViewItem><input type="text"
												id="reapu" name="pay_reapu" size="20" value="<%=map.get("pay_reapu")%>" onchange="calcu()" onblur="show()">
										...
										<TR>
											...
											<TD class=gridViewItem><input type="text" name="pay_sub"
												id="sub" size="20" value="<%=map.get("pay_sub") %>" >
										...
										<TR>
											...
											<TD class=gridViewItem><input type="text"
												name="pay_remark" size="20" value="<%=map.get("pay_remark") %>" onblur="show()">
