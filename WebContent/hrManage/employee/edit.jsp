			...
			<TBODY>
				<TR
					...
					<TD><SPAN
						...
						style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(<%=request.getContextPath()%>/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">员工信息修改</SPAN><SPAN
					...
					<TD
						...
						%>
							<form name="addForm" method="post"
								action="<%=request.getContextPath()%>/hrManage/employee.do?method=edit&id=<%=map.get("employee_id") %>"
										    ...
										    <blockquote>
										      <blockquote>
										        <blockquote>
										          <p>
										            <input type="text"
												name="employee_name" value="<%=map.get("employee_name") %>" /> 
										            <span id="text2"
									          ...
									          <blockquote>
									            <blockquote>
									              <blockquote>
									                <p>
									                  <input type="text"
												name="employee_sex" value="<%=map.get("employee_sex") %>" /> 
								                      <span id="text3"
									          ...
									          <blockquote>
									            <blockquote>
									              <blockquote>
									                <p>
									                  <input type="text"
												name="employee_brith" value="<%=map.get("employee_brith") %>" /> 
								                      <span id="text4"
									          ...
									          <blockquote>
									            <blockquote>
									              <blockquote>
									                <p>
									                  <input type="text"
												name="employee_date" value="<%=map.get("employee_date") %>" /> 
								                      <span id="text5"
												...
												<select name="position_id">
													...
													<%
														...
														for (int i = 0; i < list.size(); i++) {
															Map sort = (Map) list.get(i);
															
													%>
													<option value="<%=sort.get("depart_id") %>.<%=sort.get("position_id") %>" <%=(sort.get("depart_id") + "." + sort.get("position_id")).equals(map.get("depart_id") + "." + map.get("position_id")) ? "selected" : ""%>><%=sort.get("depart_name")%>·<%=sort.get("position_name")%></option>
													<%
														}
													%>
													<span id="text6" style="color: red; visibility: hidden">*职务id不能为空</span>
													   </blockquote>
									            </blockquote>
									          </blockquote>
									        </div>
									          ...
									          <blockquote>
									            <blockquote>
									              <blockquote><select name="employee_state">
													...
													<option value="在职" <%=map.get("employee_state").equals("在职") ? "selected" : "" %>>在职</option>
													<option value="退休" <%=map.get("employee_state").equals("退休") ? "selected" : "" %>>退休</option>
													<option value="离职" <%=map.get("employee_state").equals("离职") ? "selected" : "" %>>离职</option>
													<option value="停职" <%=map.get("employee_state").equals("停职") ? "selected" : "" %>>停职</option>
													<option value="停薪留职" <%=map.get("employee_state").equals("停职留薪") ? "selected" : "" %>>停薪留职</option>
													<option value="实习" <%=map.get("employee_state").equals("实习") ? "selected" : "" %>>实习</option>
											...
											<div align="left">
									          <blockquote>
									            <blockquote>
									              <blockquote>
											<input type="text"
												name="employee_tel" value="<%=map.get("employee_tel") %>" /> <span id="text10"
											...
											<div align="left">
									          <blockquote>
									            <blockquote>
									              <blockquote>
												  <input type="text"
												name="employee_address" value="<%=map.get("employee_address") %>" /> <span id="text11"
										...
										<TR>
											...
											<TD class=gridViewItem> <div align="left">
									          <blockquote>
												...
												name="employee_remark" value="<%=map.get("employee_remark") %>" /> <span id="text12"
